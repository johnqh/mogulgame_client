...
│   ├── MogulGameClient.ts              # HTTP client class (DI-based)
│   └── MogulGameClient.test.ts
├── hooks/
...
└── utils/
    ...
    └── mogulgame-helpers.ts            # createAuthHeaders, buildUrl, handleApiError
```
...
Hooks receive `networkClient` and `baseUrl` as the first two parameters (parameter-based DI). Each hook creates a memoized `MogulGameClient` instance internally via `useMemo`.
...
- **mogulgame_types** — Shared type definitions; this project imports all API types (`History`, request/response types, `BaseResponse`)
- **mogulgame_api** — Backend server that this client SDK communicates with over HTTP
- **mogulgame_lib** — Business logic library that consumes this client's hooks and `MogulGameClient` class
- **mogulgame_app** — Web frontend that uses this client transitively via mogulgame_lib
- **mogulgame_app_rn** — React Native app that uses this client via file: links
...
- `MogulGameClient` class accepts `{ baseUrl, networkClient }` via constructor -- never use `fetch` directly inside this package
- Hooks receive `networkClient` and `baseUrl` as the first two parameters and create a memoized `MogulGameClient` internally -- no singleton pattern
...
- Utility functions in `src/utils/mogulgame-helpers.ts` handle auth headers (`createAuthHeaders`), URL construction (`buildUrl`), and API error handling (`handleApiError`)
...
- This is a published npm package (`@sudobility/mogulgame_client`) -- breaking changes require version bumps and coordination with consumers
